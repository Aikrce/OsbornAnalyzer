...
const DeepAnalysisPage: React.FC = memo(() => {
  ...
  const { results, isLoading, error, analyze } = useDeepAnalysis();
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/30 to-indigo-50/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center py-12">
            <LoadingSpinner />
            <p className="mt-4 text-gray-600">正在生成深度分析...</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <CardContent>
                ...
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {result.marketAnalysis?.keyPlayers?.map((player, index) => (
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl shadow-gray-200/20">
              <CardHeader>
                <CardTitle className="text-xl font-bold text-gray-900 flex items-center">
                  <IconTrendingUp size={20} className="mr-3 text-orange-600" />
                  SWOT分析
                </CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-4">
                    <div className="bg-green-50 p-4 rounded-xl">
                      ...
                      <ul className="space-y-1">
                        ...
                        )) || <li className="text-gray-500 text-sm">暂无数据</li>}
                      </ul>
                    </div>
                    <div className="bg-red-50 p-4 rounded-xl">
                      ...
                      <ul className="space-y-1">
                        {result.swotAnalysis?.weaknesses?.map((weakness, index) => (
                          <li key={index} className="text-red-800 text-sm">• {weakness}</li>
                        )) || <li className="text-gray-500 text-sm">暂无数据</li>}
                      </ul>
                    </div>
                  </div>
                    ...
                    <div className="bg-blue-50 p-4 rounded-xl">
                      ...
                      <ul className="space-y-1">
                        {result.swotAnalysis?.opportunities?.map((opportunity, index) => (
                          <li key={index} className="text-blue-800 text-sm">• {opportunity}</li>
                        )) || <li className="text-gray-500 text-sm">暂无数据</li>}
                      </ul>
                    </div>
                    <div className="bg-yellow-50 p-4 rounded-xl">
                      <h4 className="font-semibold text-yellow-900 mb-2">威胁 (Threats)</h4>
                      <ul className="space-y-1">
                        {result.swotAnalysis?.threats?.map((threat, index) => (
                          <li key={index} className="text-yellow-800 text-sm">• {threat}</li>
                        )) || <li className="text-gray-500 text-sm">暂无数据</li>}
                      </ul>
                    </div>
                  </div>
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl shadow-gray-200/20">
              <CardHeader>
                <CardTitle className="text-xl font-bold text-gray-900 flex items-center">
                  <IconSparkles size={20} className="mr-3 text-purple-600" />
                  创新洞察
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-green-50 p-4 rounded-xl">
                    <h4 className="font-semibold text-green-900 mb-2">关键机会</h4>
                    <ul className="space-y-1">
                      {result.insights?.keyOpportunities?.map((opportunity, index) => (
                        <li key={index} className="text-green-800 text-sm">• {opportunity}</li>
                      )) || <li className="text-gray-500 text-sm">暂无数据</li>}
                    </ul>
                  </div>
                  <div className="bg-red-50 p-4 rounded-xl">
                    <h4 className="font-semibold text-red-900 mb-2">潜在风险</h4>
                    <ul className="space-y-1">
                      {result.insights?.potentialRisks?.map((risk, index) => (
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl shadow-gray-200/20">
              <CardHeader>
                <CardTitle className="text-xl font-bold text-gray-900 flex items-center">
                  <IconTarget size={20} className="mr-3 text-indigo-600" />
                  实施建议
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="bg-blue-50 p-4 rounded-xl">
                    <h4 className="font-semibold text-blue-900 mb-2">短期建议</h4>
                    <ul className="space-y-1">
                  ...
                  <div className="bg-green-50 p-4 rounded-xl">
                    <h4 className="font-semibold text-green-900 mb-2">中期建议</h4>
                    <ul className="space-y-1">
                  ...
                  <div className="bg-purple-50 p-4 rounded-xl">
                    <h4 className="font-semibold text-purple-900 mb-2">长期建议</h4>
                    <ul className="space-y-1">
                      {result.recommendations?.longTerm?.map((rec, index) => (
                        <li key={index} className="text-purple-800 text-sm">• {rec}</li>
                      )) || <li className="text-gray-500 text-sm">暂无数据</li>}
                    </ul>
                  </div>
                </div>
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-xl shadow-gray-200/20">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-gray-600">相似案例</span>
