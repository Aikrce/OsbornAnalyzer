...
export class PNGGenerator {
   ...
   * 绘制标题区域
   */
  private drawHeader(ctx: CanvasRenderingContext2D, title: string, description: string, width: number): void {
    const headerHeight = 120;
    
    // 标题背景渐变
    const gradient = ctx.createLinearGradient(40, 40, width - 40, 40 + headerHeight);
    ...
    const lineHeight = 30;
    const startY = 40 + headerHeight / 2 - (titleLines.length - 1) * lineHeight / 2;
   ...
   * 绘制分析内容
   */
  private drawAnalysisContent(ctx: CanvasRenderingContext2D, analysis: any, width: number, startY: number): void {
  ...
  private drawAnalysisCard(
    ...
    if (type === 'osborn') {
      ...
      if (analysis.suggestions && analysis.suggestions.length > 0) {
        ctx.fillText(`提供 ${analysis.suggestions.length} 条创新建议`, x + 20, contentY);
      }
    } else {
      if (analysis.insights) {
        const opportunityCount = analysis.insights.keyOpportunities?.length || 0;
   ...
   * 绘制底部信息
   */
  private drawFooter(ctx: CanvasRenderingContext2D, tags: string[], createdAt: string, width: number, height: number): void {
